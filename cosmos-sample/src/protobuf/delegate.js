...
const App = async () => {
	...
	cosmos2.getAccounts(address).then(data => {
		...
		const msgDelegateAny = new proto.google.protobuf.Any({
			type_url: "/cosmos.staking.v1beta1.MsgDelegate",
			value: proto.cosmos.staking.v1beta1.MsgDelegate.encode(msgDelegate).finish()
		});

		const txBody = new proto.cosmos.tx.v1beta1.TxBody({ messages: [msgDelegateAny], memo: "" });
		console.log("- msgDelegate: ", msgDelegate.toJSON())
		console.log("- msgDelegate protobuf: ", Buffer.from(proto.cosmos.staking.v1beta1.MsgDelegate.encode(msgDelegate).finish(), 'hex').toString('hex'))
		console.log('---')
		// --------------------------------- (2)authInfo ---------------------------------
